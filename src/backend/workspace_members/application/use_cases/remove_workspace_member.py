"""Use case for removing workspace members."""
...
class RemoveWorkspaceMemberRequest:
    """Remove workspace member request payload."""
...
class RemoveWorkspaceMember:
    ...
    async def execute(self, request: RemoveWorkspaceMemberRequest) -> None:
        ...
        removed = await self._workspace_member_repository.remove(
            workspace_id=request.workspace_id,
            user_id=request.user_id,
        )
        if removed:
