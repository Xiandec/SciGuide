...
`Frontend` реализует пользовательский интерфейс для работы с воркспейсами, документами и чатами на базе Vue [@vue-docs]. `Backend` на FastAPI предоставляет HTTP API, управляет прикладными сущностями и координирует сценарии загрузки документов и генерации ответов [@fastapi-docs]. `Celery`-worker выполняет фоновую индексацию документов [@celery-docs]. Библиотека `sciguide_pipeline` инкапсулирует retrieval-логику и используется как worker-ом, так и backend-частью при поиске контекста для ответа.

Хранилища распределены по функциям. `PostgreSQL` используется для пользователей, воркспейсов, чатов и сообщений [@postgresql-docs]. `Redis` служит брокером задач и кэшем [@redis-docs]. `MinIO` хранит загруженные файлы [@minio-docs]. `Qdrant` хранит векторные представления фрагментов текста [@qdrant-docs]. `Neo4j` содержит граф сущностей и их связей [@neo4j-docs]. На входе в систему размещен `Nginx`, который выступает единой точкой доступа.
...
Система рассчитана на контейнерное развертывание. Основные сервисы поднимаются через Docker Compose, что позволяет воспроизвести рабочее окружение целиком: backend, frontend, worker, базы данных и retrieval-хранилища [@docker-compose-docs]. На входе используется `Nginx`, а фоновая индексация реализована через `Celery` с `Redis` в роли брокера и backend-хранилища задач. Базовая конфигурация `Celery` приведена в `#ref(<lst-celery-config>)`{=typst}.
...
Backend поднимает подключения к `PostgreSQL` и `Redis` в `lifespan`-контексте FastAPI, а также публикует healthcheck и HTTP-метрики. Для сопровождения системы предусмотрены `Prometheus`, `Grafana`, `Loki`, `Grafana Alloy` и формат телеметрии `OpenTelemetry` [@prometheus-docs] [@grafana-docs] [@loki-docs] [@opentelemetry-docs].
...
| Embedding-модель | `BAAI/bge-m3` [@baai-bge-m3] |
...
В ходе курсового проекта была разработана автоматическая справочная система SciGuide для работы с документами выбранного научного направления. Цель работы достигнута: реализовано решение, в котором ответы формируются с опорой на загруженный корпус, а retrieval-слой сочетает векторный поиск, графовое представление сущностей и обучаемое перерангирование.

В аналитическом разделе были рассмотрены особенности научных корпусов и сформулированы требования к системе. Показано, что для такой задачи недостаточно простого поиска по ключевым словам: запросы короткие, документы содержательно насыщены, а релевантность часто определяется не только совпадением терминов, но и связями между понятиями. На этой основе был выбран двухэтапный retrieval-подход.

В практической части спроектирована и реализована архитектура SciGuide. Исследовательский уровень представлен Jupyter Notebook, общее вычислительное ядро выделено в библиотеку `sciguide_pipeline`, а прикладной уровень реализован как сервис с воркспейсами, документами, чатами и изоляцией пользовательского контекста. Для обработки документов предусмотрена фоновая индексация, а для хранения данных используются специализированные хранилища: PostgreSQL, MinIO, Qdrant и Neo4j.

В экспериментальной части выполнена оценка retrieval-подходов на датасете SciFact. Семантический поиск улучшил качество по сравнению с BM25: `nDCG@10` вырос с 0,6100 до 0,6488, а `Recall@10` — с 0,7367 до 0,7811. Добавление графового перерангирования дало дополнительный прирост до `nDCG@10 = 0,6537` и `Recall@10 = 0,7968`, что подтвердило полезность структурного сигнала как слабой поправки к embedding-поиску.

Также были разработаны и сравнены три обучаемые графовые модели: GraphSAGE, GAT и R-GCN. Лучший результат показала R-GCN, учитывающая разные типы отношений в гетерогенном графе. После подбора гиперпараметров модель достигла `nDCG@10 = 0,6754`, `Recall@10 = 0,8039` и `MRR@10 = 0,6402`, улучшив baseline по основным метрикам ранжирования.

Ограничением подхода является то, что граф сущностей строится детерминированно на основе извлечения сущностей, а не обучается совместно с моделью. Поэтому качество графового сигнала напрямую зависит от качества выделения сущностей и связей совместной встречаемости.

Таким образом, все задачи, поставленные во введении, выполнены: требования к системе определены, архитектура спроектирована, библиотечное ядро и прикладной сервис реализованы, исследовательские сценарии проведены, а применимость решения показана на выбранном экспериментальном наборе данных. Разработанный подход показывает, что для научной справочной системы наиболее оправдана схема, в которой semantic retrieval обеспечивает основной отбор кандидатов, графовый сигнал уточняет выдачу, а обучаемая R-GCN-модель используется как дополнительный слой перерангирования.
