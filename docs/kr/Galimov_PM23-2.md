...
Особенности научного корпуса хорошо видны на примере датасета SciFact, который используется в работе как основной набор данных для проверки retrieval-сценария. В ноутбуке `scifact_dataset_eda.ipynb` были получены базовые характеристики корпуса. Датасет содержит 5183 документа, 1109 запросов и 300 запросов с тестовой разметкой `qrels`; общее число размеченных пар запрос-документ равно 339. Средняя длина текста документа составляет 201,81 слова, медианная — 192 слова, а средняя длина запроса — 12,38 слова. Для размеченных запросов медианное число релевантных документов равно 1, а максимальное — 5.

![Обзор распределений SciFact](static/scifact_eda_overview.png)
...
На верхнем уровне система состоит из web-клиента, backend-сервиса, фонового worker-процесса, общей библиотеки пайплайнов и набора специализированных хранилищ. Общая структура показана на C4-диаграмме.

![C4-диаграмма системы SciGuide](static/archon-c4-sciguide.png)
...
Библиотека `sciguide_pipeline` выделена в отдельный технический компонент, чтобы не дублировать retrieval-логику в ноутбуке и в сервисе. Пользователь библиотеки работает с одной точкой входа — `PipelineManager`, который предоставляет доступ к двум фасадам: `chunking` и `search`.
...
Backend реализован в соответствии с принципами clean architecture и разделен на доменные области: `auth`, `workspaces`, `workspace_documents`, `workspace_prompt`, `chats`, `messages`, `workspace_members`. Каждая область содержит собственные слои `domain`, `application`, `infrastructure` и `presentation`. Компоновка HTTP API выполняется на уровне общего роутера:
...
Индексация документа реализована как отдельный сценарий использования. После загрузки файл читается из хранилища, для него обновляется статус обработки, затем вызывается индексатор на основе `PipelineManager`, а по завершении документ переводится в состояние `PROCESSED`.
